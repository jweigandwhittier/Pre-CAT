...
class BrukerData:
    ...
    def GenerateKspace(self):
        ...
        elif('cestsegCSUTE' in self.method["Method"]):
...
def ReadExperiment(path, ExpNum):
    ...
    if os.path.exists(reco_path):
        data.reco = ReadParamFile(reco_path)
...
def ReadTraj(filepath, reco):
    with open(filepath, "r") as f:
        ...
        pad = ((0,1),(0,0),(0,0))
        traj = np.pad(traj, pad, mode='constant', constant_values = 0)
        traj = traj.astype('complex64')
    ...
    with open(filepath, "r") as f:
        while True:
            try:
                ...
                if not line:
                    break
                # skip trying to load more acq params if sw version is pv360, gets stuck on pulse shape
                if 'ACQ_sw_version' in param_dict.keys():
                    if param_dict['ACQ_sw_version'] == '<PV-360.3.2>':
                        break
...
def ParseArray(current_file, line):
    ...
    try:
        ...
        try:
            ...
            print("Error encountered reading in %s, continuing..." % current_file)
            return None
